...
- `AGENTS.md` in the target repo root — self-contained master context file (Layer 3)
- `CLAUDE.md` in the target repo root — pointer into skill layer (Layer 3)
- `.github/copilot-instructions.md` in the target repo — condensed context (Layer 3)
- `.cursorrules` in the target repo root — copy of AGENTS.md content (Layer 3)
...
Mention directory names for navigation but do NOT list specific file paths — describe the pattern so the agent can find the right file via grep.]
...
#### `AGENTS.md` (self-contained, ~4k tokens)

This is the master context file. It must be SELF-CONTAINED — an agent reading only this file gets a useful orientation. Platforms: Codex, Zed, JetBrains, Aider, Factory.
...
## Key Conventions
[Top 5-7 conventions that affect code quality. Each: pattern + example file path.]
...
**If `AGENTS.md` already exists:** Read it first. If it contains project-specific rules or conventions not captured in the skill layer, preserve them. Add the skill layer routing alongside existing content.

#### `CLAUDE.md` (routing hub, ~2-3k tokens)

This loads into EVERY Claude Code conversation and SURVIVES context compaction. It MUST contain the full Module Routing and Task Routing tables — these are what make the skill layer discoverable. Do NOT put routing in orientation.md or any file that gets compacted. Do NOT strip routing tables in later phases in the name of "deduplication" — routing tables are the entry point mechanism, not duplicated content.
...
Full skill layer: `.ai/skills/` — read `.ai/skills/orientation.md` for system overview.
Domain context: `.ai/skills/domain-context.md` (business domain, glossary, regulatory).
...
**Critical:** The Module Routing, Task Routing, and Skill Maintenance sections are the most important content in this file. They ensure the agent loads the right skill for every task (even after compaction), and that skills continuously improve through use.

**If `CLAUDE.md` already exists:** Read it first. Preserve existing project-specific content (custom rules, caveats). Add the skill layer pointer alongside it.

#### `.github/copilot-instructions.md` (condensed, ~4k tokens)

Condensed version of AGENTS.md content, adapted for Copilot's context format. **MUST include the Module Routing and Task Routing tables** — these are the routing mechanism, not optional prose. Without them, the Copilot agent has no way to discover which skill to load.

**If this file exists AND Phase 0 scored it high-confidence:** Do not rewrite. Add the routing tables and skill layer links if missing, plus any missing conventions.
**If this file exists AND Phase 0 scored it medium/low-confidence:** Rewrite from scratch using AGENTS.md content with routing tables.
**If this file does not exist:** Create it with condensed AGENTS.md content including routing tables.

#### `.cursorrules` (copy of AGENTS.md)

Write the same content as AGENTS.md. Cursor's legacy format reads this file.

**If `.cursorrules` already exists:** Read it. If it has project-specific rules, merge them with the AGENTS.md content.
...
- [ ] CLAUDE.md is under ~2-3k tokens (must include routing tables)
